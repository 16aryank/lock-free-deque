...
public:
    ...
    void push_bottom(T x) {
        ...
        auto a = std::atomic_load_explicit(&active_array_, std::memory_order_acquire);
        ...
        if (size >= a->size() - 1) {
            ...
            std::atomic_store_explicit(&active_array_, new_array, std::memory_order_release);
    ...
    StealResult<T> steal() {
        ...
        auto old_array = std::atomic_load_explicit(&active_array_, std::memory_order_acquire);
...
private:
    ...
    std::shared_ptr<CircularArray<T>> active_array_;
